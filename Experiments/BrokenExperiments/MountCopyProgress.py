...
import subprocess
import sys
import os.path # used to make folders and verify paths
import platform # used to check the current OS
from kivy.properties import NumericProperty

all_drives = ' '
first_check = 1
#first_check_progress=0
first_check_progress = NumericProperty(0)
    ...
    ScreenManager:
        ...
        Screen:
            ...
            BoxLayout:
                ...
                padding: '150dp'
                MDRaisedButton:
                    text: "Check if a drive has been mounted"
                    size_hint: None, None
                    size: 3 * dp(48), dp(48)
                ...
                MDProgressBar:
                    id: first_check_progress
                    value: first_check_progress #app.first_check_func(1)
...
class KitchenSink(App):
    ...
    def first_check_func(self, readupdate):
        global first_check_progress
        if readupdate is 0:
            first_check_progress=first_check_progress+4
        else:
            #read return progress
            return first_check_progress
        ins = KitchenSink()
        ins.bind(first_check_progress=first_check_func)

    def show_example_dialog(self):
        if platform.system() is 'Windows':
            #be aware that all_drives must be defined as ' ' outside of the function before it will be called
            global all_drives
            #be aware that first_check must be defined as 1 outside of the function before it will be called
            global first_check
            #global first_check_progress
            #self.first_check_progress=1
            #there are no new drives at the launch
            new_drive=0
            for current_drive in ascii_uppercase:
                print 'current_drive: ', current_drive, '\n first_check_progress: ', first_check_progress
                #first_check_progress=first_check_progress+4
                self.first_check_func(0)
                print 'current_drive: ', current_drive, '\n first_check_progress: ', first_check_progress
                if not new_drive:
                    if first_check and current_drive not in all_drives and os.path.isdir(current_drive+':'):
                        #check for drives the first time the dialog is opened
                        #need to find a way to execute this code to run when Kivy opens
                        all_drives=all_drives+' '+current_drive
                        #scr_mngr.first_check_progress.value=scr_mngr.first_check_progress.value+4
                        #need to ask the user if the current drive is a target or source drive
                        content = MDLabel(font_style='Body1',
                                      theme_text_color='Secondary',
                                      text= "These drives were mounted at CFW startup:"+all_drives,
                                      valign='top')
                    elif not first_check and current_drive not in all_drives and os.path.isdir(current_drive+':'): 
                       #a drive has been inserted after the first check
                       #update all_drives to include the new drive
                       all_drives_temp=' '
                       for drive_letter in ascii_uppercase:
                            if drive_letter in current_drive or drive_letter in all_drives:
                                all_drives_temp=all_drives_temp+' '+drive_letter
                       all_drives=all_drives_temp
                       #need to ask the user if the current drive is a target or source drive
                       content = MDLabel(font_style='Body1',
                                      theme_text_color='Secondary',
                                      text="The " + current_drive + ":\ drive has just been mounted.\n"
                                           "That's pretty awesome right!\n"
                                           "These drives are currently mounted:"+all_drives,
                                      valign='top')
                       #copy all the data off of the drive
                       return_code = subprocess.check_output("c:/cygwin64/bin/run.exe rsync -avzh /cygdrive/" + current_drive + " /cygdrive/s/CinemaFlowWorks1", stderr=subprocess.STDOUT, shell=True)
                      #flag that a new drive was mounted
                       new_drive=1
                    elif not first_check :
                      # no new drives since the last check
                      content = MDLabel(font_style='Body1',
                                      theme_text_color='Secondary',
                                      text="Nothing has been mounted since the last time you asked. \n\n"
                                           "These drives are currently mounted:"+all_drives,
                                      valign='top')
            first_check=0
        else:
            content = MDLabel(font_style='Body1',
                                      theme_text_color='Secondary',
                                      text="You are using " + platform.system() + ".\n"
                                           "This function is currently designed for Windows",
                                      valign='top')
        content.bind(size=content.setter('text_size'))
        self.dialog = MDDialog(title="Results:",
                               content=content,
                               size_hint=(.8, None),
                               height=dp(200),
                               auto_dismiss=False)
